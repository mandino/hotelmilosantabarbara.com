				...
				<div class="whats-hot">
					...
					<div>
						<ul>
							...
							<?php query_posts('post_type=post&posts_per_page=2&offset=1&cat=-10'); if(have_posts()) : while(have_posts()) : the_post(); $imgsrc = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), "Image 240x161"); ?>

							<li style="width: 240px; height: 161px; background-image: url('<?= $imgsrc[0]; ?>');">
								<a href="<?php the_permalink(); ?>"><img src='<?= $imgsrc[0]; ?>' alt='<?php the_title(); ?>' style="visibility: hidden;" /></a>
						...
						<?php query_posts('post_type=post&posts_per_page=1&cat=-10'); if(have_posts()) : while(have_posts()) : the_post(); $imgsrc = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), "Image 540x361"); ?>
						<li style="width: 540px; height: 361px; background-image: url('<?= $imgsrc[0]; ?>');">
								<a href="<?php the_permalink(); ?>"><img src='<?= $imgsrc[0]; ?>' alt='<?php the_title(); ?>' style="visibility: hidden;" /></a>
						...
						<ul>
							...
							<?php query_posts('post_type=post&posts_per_page=2&offset=3&cat=-10'); if(have_posts()) : while(have_posts()) : the_post(); $imgsrc = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), "Image 240x161"); ?>
