jQuery( document ).ready( function( $ ) {
	...
	$widget_wrapper
		.on( 'click', '.tribe-mini-calendar-nav-link', function( e ) {
			...
			$('.upcoming-events-img').removeClass('hide');
			...
			$.post(
				...
				function( response ) {
					...
					if ( response.success ) {
						...
						/* update neighborhood tiles */
						var params = {
							action   : 'upcoming_event_tiles',
							current  : $('#tribe-mini-calendar-month-0').html(),
							next     : $('.next-month').data('month'),
							prev     : $('.prev-month').data('month'),
						};
						...
						$.get(
							...
							function( response ) {
								...
								$('.upcoming-events-img').addClass('hide');
								$('ul#upcoming-events').html(response);
							}
						);
					}
				}
